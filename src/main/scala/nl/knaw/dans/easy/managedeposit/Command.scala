...
import nl.knaw.dans.easy.managedeposit.commands.{ CleanDeposit, Curation, DepositReport }
...
object Command extends App with DebugEnhancedLogging {
  ...
  implicit val printStream: PrintStream = Console.out
  ...
  val result: Try[FeedBackMessage] = commandLine.subcommands match {
    case commandLine.reportCmd :: (full @ commandLine.reportCmd.fullCmd) :: Nil =>
      DepositReport(configuration).createFullReport(full.depositor.toOption, full.datamanager.toOption, full.age.toOption)
    ...
    case (clean @ commandLine.cleanCmd) :: Nil =>
      ...
      if (clean.force() || cleanInteraction())
        CleanDeposit(configuration).cleanDeposits(deleteParams)
      else
        Try { "Clean operation aborted by user" }
    case (syncFedora @ commandLine.`syncFedoraState`) :: Nil =>
      Curation(configuration).syncFedoraState(syncFedora.easyDatasetId())
