...
import nl.knaw.dans.easy.managedeposit.{ Configuration, DeleteParameters, ReportGenerator }
import nl.knaw.dans.lib.error._

import scala.util.Try

class CleanDeposit(depositPropertiesFactory: DepositPropertiesRepository)
                  (implicit dansDoiPrefixes: List[String], printStream: PrintStream) {

  def cleanDeposits(deleteParams: DeleteParameters): Try[FeedBackMessage] = {
    for {
      toBeDeletedDeposits <- depositPropertiesFactory.listDepositsToBeCleaned(deleteParams)
      deletedDepositInfos = toBeDeletedDeposits.flatMap(_.deleteDeposit(deleteParams).unsafeGetOrThrow)
    } yield {
