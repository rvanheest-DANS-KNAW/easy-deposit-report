...
import nl.knaw.dans.easy.managedeposit.{ DepositInformation, Location, NotReadableException, State }
...
class FileDepositPropertiesSpec extends TestSupportFixture
  ...
  "properties" should "return the contents of deposit.properties as a key-value map, with an added property 'depositId' -> <deposit dir name>" in {
    val dp = new FileDepositProperties(depositOne, Location.SWORD2)
  ...
  it should "return an empty properties file if there is not a deposit.propertiesFile available" in {
    val dp = new FileDepositProperties(depositDirWithoutProperties, Location.SWORD2)
  ...
  "hasDepositor" should "return true if the specified depositor is listed in the deposit.properties" in {
    new FileDepositProperties(ruimteReis02, Location.SWORD2).hasDepositor(Some("user001")) shouldBe true
  }

  it should "return true if no depositor is specified" in {
    new FileDepositProperties(ruimteReis02, Location.SWORD2).hasDepositor(None) shouldBe true
  }

  it should "return false if the specified depositor is different from the one listed in the deposit.properties" in {
    new FileDepositProperties(ruimteReis02, Location.SWORD2).hasDepositor(Some("another")) shouldBe false
  }

  it should "return true if the deposit.properties doesn't list a depositor" in {
    new FileDepositProperties(depositWithoutDepositor, Location.SWORD2).hasDepositor(Some("user001")) shouldBe true
  }

  "hasDatamanager" should "return true if the specified datamanager is listed in the deposit.properties" in {
    new FileDepositProperties(ruimteReis02, Location.SWORD2).hasDatamanager(Some("easyadmin")) shouldBe true
  }

  it should "return true if no datamanager is specified" in {
    new FileDepositProperties(ruimteReis02, Location.SWORD2).hasDatamanager(None) shouldBe true
  }

  it should "return false if the specified datamanager is different from the one listed in the deposit.properties" in {
    new FileDepositProperties(ruimteReis02, Location.SWORD2).hasDatamanager(Some("another")) shouldBe false
  }

  it should "return true if the deposit.properties doesn't list a datamanager" in {
    new FileDepositProperties(depositWithoutDepositor, Location.SWORD2).hasDatamanager(Some("user001")) shouldBe true
  }

  "isOlderThan" should "return true if the given age is larger than the lastModifiedTimestamp" in {
    new FileDepositProperties(ruimteReis02, Location.SWORD2).isOlderThan(Some(2)) shouldBe true
  }

  it should "return true if the given age is equal than the lastModifiedTimestamp" in {
    new FileDepositProperties(ruimteReis02, Location.SWORD2).isOlderThan(Some(0)) shouldBe true
  }

  it should "return false if the given age is smaller than the lastModifiedTimestamp" in {
    new FileDepositProperties(ruimteReis02, Location.SWORD2).isOlderThan(Some(-2)) shouldBe false
  }

  it should "return true if no age is given" in {
    new FileDepositProperties(ruimteReis02, Location.SWORD2).isOlderThan(None) shouldBe true
  }
  ...
  "getNumberOfContinuedDeposits" should "count the number of zip parts in a deposit" in {
    new FileDepositProperties(depositOne, Location.SWORD2).getNumberOfContinuedDeposits.success.value shouldBe 2
  }

  it should "return 0 if no zip parts are present in the deposit" in {
    new FileDepositProperties(ruimteReis02, Location.SWORD2).getNumberOfContinuedDeposits.success.value shouldBe 0
  }

  it should "return 0 if the deposit doesn't exist" in {
    new FileDepositProperties(depositDir / "non-existing-deposit", Location.SWORD2).getNumberOfContinuedDeposits.success.value shouldBe 0
  }

  "getBagDirName" should "return the bag name if it is listed in the deposit.properties" in {
    new FileDepositProperties(depositOne, Location.SWORD2).getBagDirName.success.value.value shouldBe "baggy"
  }

  it should "find the bag name on file system if it's not listed in the deposit.properties" in {
    new FileDepositProperties(ruimteReis02, Location.SWORD2).getBagDirName.success.value.value shouldBe "bag"
  }

  it should "give up and return None if no bag-name is listed in the deposit.properties and no directories are present" in {
    new FileDepositProperties(ruimteReis05, Location.SWORD2).getBagDirName.success.value shouldBe empty
  }

  "getDepositInformation" should "read the properties required for generating a report" in {
    new FileDepositProperties(depositOne, Location.INGEST_FLOW).getDepositInformation.success.value should matchPattern {
  ...
  "setCurationParameters" should "store curation properties in deposit.properties" in {
    new FileDepositProperties(ruimteReis01, Location.INGEST_FLOW).setCurationParameters(dansDoiRegistered = true, State.FEDORA_ARCHIVED, "deposit is curated") shouldBe a[Success[_]]
